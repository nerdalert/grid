...
|---|---|---|---|---|---|
| Provider traffic selection and round-robin | `cargo xtask env run-grid-provider-traffic-qualification --forge-config tests/e2e/topologies/grid-provider-traffic/forge.yaml --full --teardown --evidence-dir "$EVIDENCE_DIR"` | `tests/e2e/topologies/grid-provider-traffic/forge.yaml` | Release qualification | Routing, provider candidates, overlay selection policy, provider attribution, or gateway compatibility changes | [README](../tests/e2e/topologies/grid-provider-traffic/README.md) |
| Static weighted provider selection | `GRID_XTASK_SIM_IMAGE=ghcr.io/llm-d/llm-d-inference-sim:v0.10.2 cargo xtask env run-grid-static-weighted-qualification tests/e2e/topologies/grid-static-weighted/forge.yaml --quick --teardown --evidence-dir "$EVIDENCE_DIR"` | `tests/e2e/topologies/grid-static-weighted/forge.yaml` | Release qualification | `weightedRandom`, static placement, provider capacity, weighted overlay, or cross-site capacity propagation changes | [README](../tests/e2e/topologies/grid-static-weighted/README.md) |
| Distributed token quota | `cargo xtask env run-grid-token-rate-limit-qualification --forge-config tests/e2e/topologies/grid-token-rate-limit/forge.yaml --image-tag "$IMAGE_TAG" --evidence-dir "$EVIDENCE_DIR"` | `tests/e2e/topologies/grid-token-rate-limit/forge.yaml` | Experimental integration qualification | Quota, identity, Basic Auth, Valkey, gateway image, or shared-consumer changes | [README](../tests/e2e/topologies/grid-token-rate-limit/README.md) |
| Single-cluster multi-gateway | `cargo xtask env run-grid-single-cluster-multi-gateway-qualification --forge-config tests/e2e/topologies/grid-single-cluster-multi-gateway/forge.yaml --evidence-dir "$EVIDENCE_DIR"` | `tests/e2e/topologies/grid-single-cluster-multi-gateway/forge.yaml` | Release qualification | Same-site gateway lifecycle, overlay delivery, provider selection, concurrency, or NetworkPolicy behavior changes | [README](../tests/e2e/topologies/grid-single-cluster-multi-gateway/README.md) |
| Combined-site lifecycle | `cargo xtask env run-grid-combined-site-demo --forge-config tests/e2e/topologies/grid-combined-site/forge.yaml --full --teardown --evidence-dir "$EVIDENCE_DIR"` | `tests/e2e/topologies/grid-combined-site/forge.yaml` | Release qualification | Combined-site routing, provider add/remove/re-add, session fallback, rollout, trust, or lifecycle changes | [README](../tests/e2e/topologies/grid-combined-site/README.md) |
| GLB | `cargo xtask env run-grid-glb-demo --forge-config tests/e2e/topologies/grid-glb-demo/forge.yaml --full --teardown --evidence-dir "$EVIDENCE_DIR"` | `tests/e2e/topologies/grid-glb-demo/forge.yaml` | Release qualification | GLB, ingress, edge/provider boundary, mTLS, or network-boundary changes | [README](../tests/e2e/topologies/grid-glb-demo/README.md) |
| Workload inference / no ingress | `cargo xtask env run-grid-glb-demo --forge-config tests/e2e/topologies/grid-glb-demo/forge.yaml --no-ingress --full --teardown --evidence-dir "$EVIDENCE_DIR"` | Reuses `tests/e2e/topologies/grid-glb-demo/forge.yaml`; documented by `tests/e2e/topologies/grid-workload-inference/README.md` | Release qualification | Workload inference, no-ingress routing, or cluster-local workload entry changes | [README](../tests/e2e/topologies/grid-workload-inference/README.md) |
| llm-d pool metrics pressure and recovery | `cargo xtask env run-grid-llmd-pool-metrics-demo --forge-config tests/e2e/topologies/grid-llmd-pool-metrics/forge.yaml --full --teardown --evidence-dir "$EVIDENCE_DIR"` | `tests/e2e/topologies/grid-llmd-pool-metrics/forge.yaml` | Experimental integration qualification | EPP metrics, queue/KV-cache scoring, pressure transitions, recovery, or metrics mTLS changes | [README](../tests/e2e/topologies/grid-llmd-pool-metrics/README.md) |
...
GRID_XTASK_SIM_IMAGE=ghcr.io/llm-d/llm-d-inference-sim:v0.10.2 cargo xtask env run-grid-static-weighted-qualification tests/e2e/topologies/grid-static-weighted/forge.yaml --quick --teardown --evidence-dir "$EVIDENCE_DIR"
...
The checked-in Praxis configurations for these Kind qualifications explicitly
enable both `insecure_options.allow_private_endpoints` and
`insecure_options.allow_private_upstreams` because their test backends use
private Kind Service addresses and reserved cluster IPs. These are
qualification-only settings; production configurations retain the default
